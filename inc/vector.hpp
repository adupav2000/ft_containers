...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/12 17:00:36 by adu-pavi          #+#    #+#             */
/*   Updated: 2022/05/12 20:44:17 by adu-pavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	class vector
	{
		...
		private:
			void _alloc_cpy(size_type cap)
			{
				pointer 	buffer = _alloc.allocate(cap);
			...
			void _alloc_re(void)
			{
				...
				if (!_capacity) cap = 1;
				if (cap > max_size()) throw std::exception();
				_alloc_copy(cap);
			}
		...
		public:
			...
			{
				difference_type int_pos = position - begin();
				if (capacity() == size())
					_alloc_re();
